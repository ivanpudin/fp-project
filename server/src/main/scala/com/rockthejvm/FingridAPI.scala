...
// shared interface
trait EnergyClient {
  def fetch(datasetId: Int, source: String, startTime: ZonedDateTime, endTime: ZonedDateTime): Either[String, List[EnergyReading]]
}
...
object FingridClient extends EnergyClient {
  ...
  @annotation.tailrec
  private def fetchAllPages(datasetId: Int, source: String, start: ZonedDateTime, end: ZonedDateTime, page: Int = 1, acc: List[EnergyReading] = List.empty): Either[String, List[EnergyReading]] = {
...
// local writing a file in a .csv format
object FileIO {
